...
async function renderAdmin() {
  ...
  async function cargar() {
    ...
    const filas = (rows.data || []).map(filaAdmin).join('') ||
  ...
  selAnio.addEventListener('change', () => { sel.anio = Number(selAnio.value); cargar(); });
  selEstado.addEventListener('change', () => { sel.estado = selEstado.value; cargar(); });
  await cargar();
}
...
async function renderHijos() {
  ...
  selEl.addEventListener('change', () => { ctx.selectedChildId = Number(selEl.value); cargarTabla(); });
  async function cargarTabla() {
    loading(body);
    try {
      const res = await api('/pagos', { query: { estudiante_id: ctx.selectedChildId, limit: 500 } });
      const filas = (res.data || []).map(filaTutor).join('') ||
        '<tr><td colspan="6"><div class="empty">Sin cobros registrados.</div></td></tr>';
      crumbs.textContent = `Pagos de ${hijos.find((h) => h.id === ctx.selectedChildId)?.nombre || ''}`;
...
async function abrirFormCobro(id) {
  ...
  const body = openModal(esEdicion ? 'Editar cobro' : 'Generar cobro', `
    <div class="form-grid">
      <div class="field full"><label>Estudiante *</label>
        <select id="p-estudiante">${estudiantesList.map((e) =>
          `<option value="${e.id}" ${pago && pago.estudiante_id === e.id ? 'selected' : ''}>${esc(e.nombre)} ${esc(e.apellido)}${e.grado_nombre ? ` · ${esc(e.grado_nombre)}${esc(e.seccion || '')}` : ''}</option>`).join('')}</select>
      </div>
      <div class="field"><label>Concepto *</label>
        <select id="p-concepto">${conceptosList.map((c) =>
          `<option value="${c.id}" data-monto="${c.monto_sugerido ?? ''}" ${pago && pago.concepto_id === c.id ? 'selected' : ''}>${esc(c.nombre)}</option>`).join('')}</select>
      </div>
  ...
  conceptoEl.addEventListener('change', () => {
    if (montoTocado) return;
    montoEl.value = conceptoEl.selectedOptions[0]?.dataset.monto || '';
  });
  if (!pago && conceptoEl.value) montoEl.value = conceptoEl.selectedOptions[0]?.dataset.monto || '';

  body.querySelector('[data-cancel]').addEventListener('click', closeModal);
  body.querySelector('[data-save]').addEventListener('click', async () => {
    const data = {
      estudiante_id: Number(formValue('p-estudiante')),
      concepto_id: Number(formValue('p-concepto')),
